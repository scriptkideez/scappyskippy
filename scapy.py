...
root.title('Scapy Spooky Scanny')

tk.Label(root, text='IPs Scanning', font="Helvetica 24 bold").pack()
tk.Label(root, text="Enter Your IP", font="Helvetica 16 bold").pack()

subdomain_entry = tk.Entry(root)
subdomain_entry.pack(ipady=5, ipadx=50, pady=10)

treev = ttk.Treeview(root, height=300)
treev.column('#0')

button_frame = tk.Frame(root)

tk.Button(button_frame, text='Start Scanning', command=start_button, width=15, font="Helvetica 16 bold").pack(side=tk.LEFT)
tk.Button(button_frame, text='Stop Scanning', command=stop_button, width=15, font="Helvetica 16 bold").pack(side=tk.LEFT)
